...
class FSDPWorker(Worker):
    ...
    def _init_config(
        ...
        if self.config.rollout.n > 1:
            config.global_batch_size *= self.config.rollout.n+self.config.rollout.anchor_n
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def update_factor(self, kl_coef_tensor, scale_factor_tensor):
        assert self._is_actor

        self.actor.kl_coef_tensor = kl_coef_tensor
        self.actor.scale_factor_tensor = scale_factor_tensor

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def update_adv_estimator(self, adv_estimator):
        assert self._is_actor
        self.actor.adv_estimator = adv_estimator
    ...
    def update_actor_advantage(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_actor_advantage", logger=None) as timer:
                new_advantages = self.actor.update_actor_advantage(data=data)
        ...
        adv_cpu = new_advantages.detach()
        batch_size = adv_cpu.shape[0] # TensorDict 需要显式指定 batch_size

        output = DataProto(
            batch=TensorDict(
                source={"advantages": adv_cpu},
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            #         # .detach() 去除梯度, .cpu() 移到内存, .numpy() 转为 numpy 数组
            #         # 如果是标量 Tensor (0-d), .numpy() 会得到一个 0-d array
            #         return v.detach().cpu()
            #     return v
            ...
            #     # 先把 value 清洗一遍，变成 CPU 上的 numpy 或 python scalar

            #     print(f"key: {key}")
            #     print(f"type: {type(value)}")

            #     safe_val = to_safe_numpy(value)
                
            #     # 逻辑说明：
            #     # 如果 safe_val 是标量（Python float/int 或 0-d numpy array），加上 [] 变成 1-d
            #     # 否则直接用原值
            #     if np.isscalar(safe_val):  # 比 np.isscalar 更通用，能处理 0-d array
            #         final_dict[key] = np.array([safe_val])
            #     else:
            #         final_dict[key] = np.array(safe_val)
            
            # output = DataProto(
            #     non_tensor_batch=final_dict
            # )
